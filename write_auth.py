...
Run this script once from your project root:
    python write_auth.py

It will write app/middleware/auth.py directly with no copy-paste corruption.
"""
import os
...
async def get_or_create_user(payload: dict) -> dict:
    ...
    if not tenant:
        ...
        tenant_result = supabase.table("tenants").insert({"name": tenant_name, "domain": domain, "plan": "demo"}).execute()
        tenant = tenant_result.data[0]
    user_result = supabase.table("users").insert({
        "auth0_id": auth0_id, "email": email, "name": name, "picture": picture,
        "tenant_id": tenant["id"], "role": "compliance_officer", "last_login": "NOW()",
    }).execute()
...
def require_min_role(min_role):
    ...
    async def role_checker(
        ...
        if ROLE_HIERARCHY.get(user.role, 0) < ROLE_HIERARCHY.get(min_role_str, 0):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Requires role: {min_role_str}. Your role: {user.role}")
        return user
