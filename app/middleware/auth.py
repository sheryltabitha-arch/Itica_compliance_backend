...
Compatible with existing routers using CurrentUser, require_min_role, UserRole.
"""
...
def require_min_role(min_role):
    # Accept both string and Enum values (e.g. UserRole.compliance_officer)
    ...
    async def role_checker(
        ...
        required_level = ROLE_HIERARCHY.get(min_role_str, 0)
    ...
    return role_checker    for key in _jwks_cache.get("keys", []):
        if key.get("kid") == kid:
            return key
    return None
...
def require_min_role(min_role: str):
    ...
    Usage: Depends(require_min_role("admin"))
    Roles in order: analyst < compliance_officer < admin
    """
