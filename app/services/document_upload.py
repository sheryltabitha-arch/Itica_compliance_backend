...
async def upload_document(
    ...
    try:
        result = sb.table("kyc_documents").insert({
            "id":               document_id,
        ...
        # PGRST204 means "no rows returned" which is normal for an INSERT with no RETURNING clause,
        # but if result.data is None and there's an APIError, it will raise before here.
        # Log row count so insert failures surface in logs rather than passing silently.
        logger.info(f"kyc_documents insert ok: document_id={document_id}, tenant_id={current.tenant_id}")
    except Exception as e:
        # Log at ERROR so this is visible — a silent failure here means extractions
        # will reference a document_id with no metadata row, causing PGRST204 on join queries.
        logger.error(f"kyc_documents table insert failed: {e!r} | document_id={document_id} | tenant_id={current.tenant_id}")
    ...
    try:
        ...
        logger.error(f"AUDIT GAP: audit_events insert failed for document {document_id}, "
        ...
        try:
            ...
            logger.critical(f"AUDIT GAP UNRECOVERABLE: could not log failure to fallback table "
                             f"for document {document_id}, tenant {current.tenant_id}")
