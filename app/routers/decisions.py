...
async def create_decision(payload: DecisionCreate, user=Depends(get_current_user)):
...
@router.get("/")
async def list_decisions(limit: int = 50, offset: int = 0, user=Depends(get_current_user)):
    supabase = get_supabase()
    ...
    return {"decisions": result.data, "total": count_result.count or 0, "tenant_id": tenant_id}
...
async def get_stats(user=Depends(get_current_user)):
    ...
    decisions_count = supabase.table("decisions").select("id", count="exact").eq("tenant_id", tenant_id).execute()
    kyc_count = supabase.table("kyc_documents").select("id", count="exact").eq("tenant_id", tenant_id).execute()
    kyc_verified = supabase.table("kyc_documents").select("id", count="exact").eq("tenant_id", tenant_id).eq("status", "VERIFIED & LEDGERED").execute()
    audit_count = supabase.table("audit_events").select("id", count="exact").eq("tenant_id", tenant_id).execute()
