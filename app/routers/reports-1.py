...
async def generate_report_docx(
    ...
    ext_query = sb.table("extractions").select("*").eq("tenant_id", tenant_id)
    if period_start:
        ext_query = ext_query.gte("created_at", period_start)
    if period_end:
        ext_query = ext_query.lte("created_at", period_end)
    rows = ext_query.execute().data or []

    evt_query = sb.table("audit_events").select("*").eq("tenant_id", tenant_id).order("created_at", desc=True)
    if period_start:
        evt_query = evt_query.gte("created_at", period_start)
    if period_end:
        evt_query = evt_query.lte("created_at", period_end)
    events = evt_query.execute().data or []

    dec_query = sb.table("decisions").select("id", count="exact").eq("tenant_id", tenant_id)
    if period_start:
        dec_query = dec_query.gte("created_at", period_start)
    if period_end:
        dec_query = dec_query.lte("created_at", period_end)
    decisions = dec_query.execute()
