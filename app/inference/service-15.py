...
def _parse_fields(raw_fields: dict) -> dict[str, Any]:
    ...
    if given_val and surname_val:
        ...
        given_conf = getattr(given, "confidence", None)
        surname_conf = getattr(surname, "confidence", None)
        given_f = float(given_conf.value) if hasattr(given_conf, "value") else float(given_conf or 0.80)
        surname_f = float(surname_conf.value) if hasattr(surname_conf, "value") else float(surname_conf or 0.80)
        confidence_scores["full_name"] = round(min(given_f, surname_f), 4)
    elif given_val:
