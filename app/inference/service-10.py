...
def _get_signed_url(storage_path: str) -> str:
    """Generate a short-lived signed URL from Supabase Storage for Eden AI to fetch."""
    from app.middleware.auth import get_supabase
    SUPABASE_STORAGE_BUCKET = "kyc-documents"
    sb = get_supabase()
    result = sb.storage.from_(SUPABASE_STORAGE_BUCKET).create_signed_url(
        storage_path, expires_in=300  # 5 minutes
    )
    url = result.get("signedURL") or result.get("signed_url") or result.get("data", {}).get("signedUrl")
    if not url:
        raise RuntimeError(f"Could not generate signed URL: {result}")
    return url


def _query_eden_ai(image_bytes: bytes, storage_path: str | None = None) -> dict:
    """
    Send document to Eden AI identity parser.
    Uses file_url for PDFs (Eden AI requirement), base64 for images.
    """
    ...
    # Eden AI requires file_url for PDFs — base64 only supported for images
    if file_type == "pdf":
        if not storage_path:
            raise RuntimeError("storage_path required for PDF extraction")
        signed_url = _get_signed_url(storage_path)
        logger.info(f"Eden AI: using file_url for PDF")
        payload = {
            "providers": PROVIDERS,
            "file_url":  signed_url,
        }
    ...
    logger.info(f"Eden AI: sending request with providers={PROVIDERS}")
...
def extract_document_fields(image_bytes: bytes, storage_path: str | None = None) -> dict[str, Any]:
    ...
    storage_path is required for PDF documents.
    """
    if not image_bytes:
    ...
    raw    = _query_eden_ai(image_bytes, storage_path=storage_path)
