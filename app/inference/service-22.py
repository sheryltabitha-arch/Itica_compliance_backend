...
def _parse_confidence(conf) -> float:
    ...
    return CONF_MAP.get(str(val).lower(), 0.80)
...
def _parse_fields(raw_fields: dict) -> dict[str, Any]:
    ...
    def extract(mindee_key: str, our_key: str):
        ...
        confidence = _parse_confidence(getattr(field, "confidence", None))
    ...
    if given_val and surname_val:
        ...
        confidence_scores["full_name"] = round(
            min(
                _parse_confidence(getattr(given, "confidence", None)),
                _parse_confidence(getattr(surname, "confidence", None)),
            ), 4
        )
    elif given_val:
